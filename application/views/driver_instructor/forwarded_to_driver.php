...
<div class="content">
    <div class="container-fluid">
        ...
        <?=form_open('PurchaseRequestCtrl/forwarded_to_driver/'.$select->purchase_request_id,  array('onsubmit'=>'return confirm(\'Are you sure you want to forward?\')'));?>
    ...
    <div class="card border-success mb-3"
        ...
        <div class="card-footer bg-transparent border-success text-end">
            ...
            <input type="submit" name="btn_forwarded" value="Forward" class="btn_save">


            <a href="<?php echo site_url('PurchaseRequestCtrl/branch_manage_pr'); ?>"><input type="button" value="Back"
                    class="btn_save"></a>

            <input class="form-control form-control-lg" type="hidden" name="contact" value="<?=$select->contact_no?>" />
            ...
            <input class="form-control form-control-lg" type="hidden" name="message"
                value="iDrive Driving Tutorial - <?=$sms_branch->branch_name.'. '.'forwarded a ' .$select->isPending?> a purchase request of <?=ucfirst($select->First_Name).' ' . ucfirst($select->Last_Name)?>" />
            <?php
            }else{

                ?>
            <input class="form-control form-control-lg" type="hidden" name="message"
                value="iDrive Driving Tutorial - <?=$sms_branch->branch_name.'. '.'forwarded a ' .$select->isPending?> a service request of <?=ucfirst($select->First_Name).' ' . ucfirst($select->Last_Name)?>" />
            <?php
            }?>
